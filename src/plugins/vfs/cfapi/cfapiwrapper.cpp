    ...
    winrt::StorageFolder::GetFolderFromPathAsync(reinterpret_cast<const wchar_t *>(nativePath.utf16()))
        .Completed([params, callback](const winrt::IAsyncOperation<winrt::StorageFolder> &result, winrt::AsyncStatus status) {
            ...
            try {
                ...
                // displayed when an error occurs "Make sue OpenCloud is running on your PC, try again." and in the trash bin etc.
                const QString displayname = u"%1: %2@%3"_s.arg(params.providerDisplayName, params.folderDisplayName(), params.account->davDisplayName());
