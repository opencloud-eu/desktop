...
void SocketApi::command_SHARE(const QString &localFile, SocketListener *listener)
{
    ...
    if (!shareFolder) {
        ...
        if (fileData.folder->accountState()->account()->capabilities().filesSharing().sharing_roles) {
            fetchPrivateLinkUrl(fileData.folder->accountState()->account(), fileData.folder->webDavUrl(), remotePath, this, [](const QUrl &url) {
