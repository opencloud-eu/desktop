...
OPENCLOUD_SYNC_EXPORT Q_DECLARE_LOGGING_CATEGORY(lcUtility)
    ...
    namespace Utility
{
     ...
     */
    OPENCLOUD_SYNC_EXPORT bool hasSystemLaunchOnStartup(const QString &appName);
     ...
     */
    OPENCLOUD_SYNC_EXPORT bool hasLaunchOnStartup(const QString &appName);
    OPENCLOUD_SYNC_EXPORT void setLaunchOnStartup(const QString &appName, const QString &guiName, bool launch);
     ...
     */
    OPENCLOUD_SYNC_EXPORT QString compactFormatDouble(double value, int prec, const QString &unit = QString());

    // porting methods
    OPENCLOUD_SYNC_EXPORT QString escape(const QString &);

    // conversion function QDateTime <-> time_t   (because the ones builtin work on only unsigned 32bit)
    OPENCLOUD_SYNC_EXPORT QDateTime qDateTimeFromTime_t(qint64 t);
    OPENCLOUD_SYNC_EXPORT qint64 qDateTimeToTime_t(const QDateTime &t);
     ...
     */
    OPENCLOUD_SYNC_EXPORT QString durationToDescriptiveString1(std::chrono::milliseconds msecs);
    OPENCLOUD_SYNC_EXPORT QString durationToDescriptiveString2(std::chrono::milliseconds msecs);
     ...
     */
    OPENCLOUD_SYNC_EXPORT bool hasDarkSystray();
    ...
    OPENCLOUD_SYNC_EXPORT QString platformName();
    // crash helper for --debug
    OPENCLOUD_SYNC_EXPORT void crash();
    ...
    OPENCLOUD_SYNC_EXPORT bool fsCasePreserving();
    ...
    // case sensitivity.
    OPENCLOUD_SYNC_EXPORT bool fileNamesEqual(const QString &fn1, const QString &fn2);
    ...
    OPENCLOUD_SYNC_EXPORT QString versionOfInstalledBinary(const QString &command = QString());

    OPENCLOUD_SYNC_EXPORT QString fileNameForGuiUse(const QString &fName);

    OPENCLOUD_SYNC_EXPORT QString normalizeEtag(QStringView etag);
     ...
     */
    OPENCLOUD_SYNC_EXPORT QString timeAgoInWords(const QDateTime &dt, const QDateTime &from = QDateTime());

    /**
     * @brief Sort a QStringList in a way that's appropriate for filenames
     */
    OPENCLOUD_SYNC_EXPORT void sortFilenames(QStringList & fileNames);

    /**
     * Concatenate parts of a URL path with a given delimiter, eliminating duplicate
     */
    OPENCLOUD_SYNC_EXPORT QString concatUrlPathItems(QStringList && items, const QLatin1Char delimiter = QLatin1Char('/'));

    /** Appends concatPath and queryItems to the url */
    OPENCLOUD_SYNC_EXPORT QUrl concatUrlPath(const QUrl &url, const QString &concatPath, const QUrlQuery &queryItems = {});

    /** Compares two urls and ignores whether thei end wit / */
    OPENCLOUD_SYNC_EXPORT bool urlEqual(QUrl a, QUrl b);

    /**  Returns a new settings pre-set in a specific group.  The Settings will be created
         with the given parent. If no parent is specified, the caller must destroy the settings */
    OPENCLOUD_SYNC_EXPORT std::unique_ptr<QSettings> settingsWithGroup(const QString &group, QObject *parent = nullptr);
     ...
     * will pass unchanged.
     */
    OPENCLOUD_SYNC_EXPORT QString sanitizeForFileName(const QString &name);

    /** Returns a file name based on \a fn that's suitable for a conflict.
     */
    OPENCLOUD_SYNC_EXPORT QString makeConflictFileName(const QString &fn, const QDateTime &dt, const QString &user);

    /** Returns whether a file name indicates a conflict file
     */
    OPENCLOUD_SYNC_EXPORT bool isConflictFile(QStringView name);
