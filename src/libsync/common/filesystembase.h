...
OPENCLOUD_SYNC_EXPORT Q_DECLARE_LOGGING_CATEGORY(lcFileSystem)
    ...
    namespace FileSystem
{
     ...
     */
    void OPENCLOUD_SYNC_EXPORT setFileHidden(const QString &filename, bool hidden);
     ...
     */
    void OPENCLOUD_SYNC_EXPORT setFileReadOnly(const QString &filename, bool readonly);
     ...
     */
    void OPENCLOUD_SYNC_EXPORT setFileReadOnlyWeak(const QString &filename, bool readonly);

    /**
     * @brief Try to set permissions so that other users on the local machine can not
     * go into the folder.
     */
    void OPENCLOUD_SYNC_EXPORT setFolderMinimumPermissions(const QString &filename);
     ...
     */
    QString OPENCLOUD_SYNC_EXPORT longWinPath(const QString &inpath);
     ...
     * files, see above.
     */
    bool OPENCLOUD_SYNC_EXPORT fileExists(const QString &filename, const QFileInfo & = QFileInfo());
     ...
     */
    bool OPENCLOUD_SYNC_EXPORT rename(const QString &originFileName, const QString &destinationFileName, QString *errorString = nullptr);
     ...
     */
    bool OPENCLOUD_SYNC_EXPORT uncheckedRenameReplace(const QString &originFileName, const QString &destinationFileName, QString *errorString);
     ...
     */
    bool OPENCLOUD_SYNC_EXPORT remove(const QString &fileName, QString *errorString = nullptr);
     ...
     */
    bool OPENCLOUD_SYNC_EXPORT isFileLocked(const QString &fileName, LockMode mode);

#ifdef Q_OS_WIN

    bool OPENCLOUD_SYNC_EXPORT longPathsEnabledOnWindows();

    /**
     * Returns the file system used at the given path.
     */
    QString OPENCLOUD_SYNC_EXPORT fileSystemForPath(const QString &path);
#endif

    /**
     * Returns whether the file is a shortcut file (ends with .lnk)
     */
    bool OPENCLOUD_SYNC_EXPORT isLnkFile(const QString &filename);

    /**
     * Returns whether the file is a junction (windows only)
     */
    bool OPENCLOUD_SYNC_EXPORT isJunction(const QString &filename);
     ...
     */
    QString OPENCLOUD_SYNC_EXPORT createPortableFileName(const QString &path, const QString &fileName, qsizetype reservedSize = 0);

    /*
     * Replace path navigation elements from the string
     */
    QString OPENCLOUD_SYNC_EXPORT pathEscape(const QString &s);
