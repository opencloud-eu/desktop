$clazy="ClazyClangTidy"

if($isWindows) {
    $clazy += ".dll"
} elseif($isLinux) {
    $clazy = "${env:KDEROOT}/lib/${clazy}.so"
} else
{
    $clazy = "${env:KDEROOT}/lib/${clazy}.dylib"
}
...
run-clang-tidy -load="${clazy}" -checks="${CLAZY_LEVEL0},-overloaded-signal,qt-keywords" -p "$env:BUILD_DIR" | Tee-Object -Path "$([System.IO.Path]::GetTempPath())/clang-tidy.log"
